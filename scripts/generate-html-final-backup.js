...
// Load FC cards as base64
const fcCards = JSON.parse(fs.readFileSync(__dirname + '/fantasy-cards-base64.json', 'utf8'));
...
function getPlayerImage(name) {
  ...
  return fcCards[key] || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&size=200&background=00e676&color=0f0f0f&bold=true`;
}

function getTeamBadge(club) {
  const key = club.toLowerCase().replace(/[^a-z0-9]/g, ' ').replace(/^\d+\s+/, '').trim();
  return teamBadges[key] || `https://ui-avatars.com/api/?name=${encodeURIComponent(club.substring(0,3))}&size=80&background=2a2a2a&color=fff`;
}
      ...
      <div class="player-header">
        ...
        <div>
          ...
          <div class="player-badges">
            <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(player.league.substring(0,3))}&size=32&background=2a2a2a&color=fff" alt="League">
            <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(clubDisplay.substring(0,3))}&size=32&background=2a2a2a&color=fff" alt="Club">
          </div>
        </div>
