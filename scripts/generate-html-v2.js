...
/**
 * Generate enhanced HTML with FUTBIN-style progress bars and fixtures
 */
...
function getPlayerImage(name) {
  // Try to get real player image from FUTBIN
...
function getTeamBadge(club) {
  ...
  if (teamBadges[club]) return teamBadges[club];
  
  // Try without numbering (e.g., '1. FC Heidenheim' -> 'FC Heidenheim')
  const cleaned = club.replace(/^d+.s+/, '');
  if (teamBadges[cleaned]) return teamBadges[cleaned];
  
  // Fallback to generated badge
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(club.substring(0, 3))}&size=80&background=2a2a2a&color=fff&format=svg`;
}
  ...
  <div class="container">
    ...
    <div class="player-card">
      <div class="player-header">
        <img src="${getPlayerImage(player.name)}" alt="${player.name}" style="width: 80px; height: 80px; border-radius: 50%; border: 3px solid #00e676;">
    ...
    for (const match of playerMatches) {
      ...
      <div class="fixture-card">
        <div class="team home"><img src="${getTeamBadge(match.club)}" alt="" style="width: 40px; height: 40px; margin-right: 10px;">${match.club}</div>
        <div>
          <div class="score">${resultIcon} ${match.score_for}-${match.score_against}</div>
          <div class="match-date">${date}</div>
        </div>
        <div class="team"><img src="${getTeamBadge(match.opponent)}" alt="" style="width: 40px; height: 40px; margin-right: 10px;">${match.opponent}</div>
    ...
    console.log('✅ Enhanced HTML generated with FUTBIN-style UI!');
