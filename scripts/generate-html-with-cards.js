#!/usr/bin/env node
// Generate HTML with real FUTBIN Fantasy FC card images
...
// Load player FUT IDs
const playerFutIds = JSON.parse(fs.readFileSync(__dirname + '/player-fut-ids.json', 'utf8'));

function getCardImage(name) {
  const futId = playerFutIds[name];
  if (!futId) {
    return null;
  }
  
  return {
    template: 'https://cdn3.futbin.com/content/fifa26/img/cards/hd/111_fantasy_fc.png?fm=png&ixlib=java-2.1.0&w=168',
    player: `https://cdn3.futbin.com/content/fifa26/img/players/p${futId}.png?fm=png&ixlib=java-2.1.0&w=168`
  };
}
...
<body>
  <div class="container">
    ...
    for (const player of playersResult.rows) {
      ...
      const card = getCardImage(player.name);
    ...
    <div class="player-card">
      ...
      if (card) {
        html += `
        <div class="fut-card">
          <img src="${card.template}" alt="Card template">
          <img src="${card.player}" alt="${player.name}">
        </div>`;
      }
    ...
    console.log('✅ HTML generated with real FUTBIN card images!');
